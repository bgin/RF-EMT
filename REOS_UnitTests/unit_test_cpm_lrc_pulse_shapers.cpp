...
/*
   icpc -o unit_test_cpm_lrc_pulse_shapers -O3 -fp-model fast=2 -std=c++17 -ftz -ggdb -ipo -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
...
void unit_test_cpm_lrc_pulse_shapers();
...
void unit_test_cpm_lrc_pulse_shapers()
{
   ...
   cpm_pulse_shapers_t   lrc_pulse = cpm_pulse_shapers_t(nTLsamples,nTsamples,
                                                            T,L,beta,BbT);
   int32_t status{};
     //__asm__ ("int3");
   printf("[UNIT_TEST]: function=%s -- **START**\n", __PRETTY_FUNCTION__);    
   char * ctor_name{gms::common::demangle(typeid(lrc_pulse).name(),status)};
   ...
   {
          printf("[UNIT-TEST]: Instantiation of object Constructor of type: %s\n\n", typeid(lrc_pulse).name());
          if(ctor_name != NULL) gms::common::gms_mm_free(ctor_name);
   }  
   printf("[UNIT-TEST]: #1 -- generate_lrc_pulse_scalar_u8x()\n");
   lrc_pulse.generate_lrc_pulse_scalar_u8x();
   std::printf("[UNIT-TEST:] -- Creating gnuplot plotting command file.\n");
   cpm_pulse_shapers_t::create_signal_plot(static_cast<std::int32_t>(lrc_pulse.m_nTLsamples),lrc_pulse.m_lrc_pulse.m_data,nullptr,
                                              "cpm_lrc_pulse_scalar_u8x_test_1_","L-Raised Cosine Pulse",false);
   lrc_pulse.clear_lrc_pulse(0.0f);
   printf("[UNIT-TEST]: #2 -- generate_lrc_pulse_sse_rolled()\n");
   lrc_pulse.generate_lrc_pulse_sse_rolled();
   std::printf("[UNIT-TEST:] -- Creating gnuplot plotting command file.\n");
   cpm_pulse_shapers_t::create_signal_plot(static_cast<std::int32_t>(lrc_pulse.m_nTLsamples),lrc_pulse.m_lrc_pulse.m_data,nullptr,
                                              "cpm_lrc_pulse_sse_rolled_test_1_","L-Raised Cosine Pulse",false);
   lrc_pulse.clear_lrc_pulse(0.0f);
   printf("[UNIT-TEST]: #2 -- generate_lrc_pulse_sse_u8x()\n");
   lrc_pulse.generate_lrc_pulse_sse_u8x();
   std::printf("[UNIT-TEST:] -- Creating gnuplot plotting command file.\n");
   cpm_pulse_shapers_t::create_signal_plot(static_cast<std::int32_t>(lrc_pulse.m_nTLsamples),lrc_pulse.m_lrc_pulse.m_data,nullptr,
                                              "cpm_lrc_pulse_sse_u8x_test_1_","L-Raised Cosine Pulse",false);
   printf("[UNIT_TEST]: function=%s -- **END**\n", __PRETTY_FUNCTION__); 
}

int main()
{
   unit_test_cpm_lrc_pulse_shapers();
   return (0);
}