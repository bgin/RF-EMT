...
void unit_test_cpm_gmsk_pulse_shapers();
...
void unit_test_cpm_gmsk_pulse_shapers()
{
   ...
   constexpr float       T{600.0f};
   constexpr float       L{3.0f};
   constexpr float       beta{0.854f};
   constexpr float       BbT{0.245f};
   cpm_pulse_shapers_t   gmsk_pulse = cpm_pulse_shapers_t(nTLsamples,nTsamples,
                                                            T,L,beta,BbT);
   int32_t status{};
     //__asm__ ("int3");
   printf("[UNIT_TEST]: function=%s -- **START**\n", __PRETTY_FUNCTION__);    
   char * ctor_name{gms::common::demangle(typeid(gmsk_pulse).name(),status)};
   ...
   {
          printf("[UNIT-TEST]: Instantiation of object Constructor of type: %s\n\n", typeid(gmsk_pulse).name());
          if(ctor_name != NULL) gms::common::gms_mm_free(ctor_name);
   }  
   printf("[UNIT-TEST]: #1 -- generate_gmsk_pulse_scalar_rolled()\n");
   ...
   gmsk_pulse.generate_gmsk_pulse_sse_rolled();
   std::printf("[UNIT-TEST:] -- Creating gnuplot plotting command file.\n");
   cpm_pulse_shapers_t::create_signal_plot(static_cast<std::int32_t>(gmsk_pulse.m_nTsamples),gmsk_pulse.m_gmsk_pulse.m_data,nullptr,
                                              "cpm_gmsk_pulse_sse_rolled_test_1_","Gaussian MSK Pulse",false);
   gmsk_pulse.clear_gmsk_pulse(0.0f);
   printf("[UNIT-TEST]: #2 -- generate_gmsk_pulse_sse_u8x()\n");
   gmsk_pulse.generate_gmsk_pulse_sse_u8x();
   std::printf("[UNIT-TEST:] -- Creating gnuplot plotting command file.\n");
   cpm_pulse_shapers_t::create_signal_plot(static_cast<std::int32_t>(gmsk_pulse.m_nTsamples),gmsk_pulse.m_gmsk_pulse.m_data,nullptr,
                                              "cpm_gmsk_pulse_sse_u8x_test_1_","Gaussian MSK Pulse",false);
   */
...
int main()
{
   unit_test_cpm_gmsk_pulse_shapers();
   return (0);
}