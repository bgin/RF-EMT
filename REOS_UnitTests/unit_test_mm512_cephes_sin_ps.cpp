...
/*
   icpc -o unit_test_mm512_cephes_sin_ps -O3 -fp-model fast=2 -fno-exceptions -std=c++17 -ftz -ggdb -ipo -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_cephes_sin_cos.h GMS_simd_cephes_sin.h unit_test_mm512_cephes_sin_ps.cpp
   ASM: 
   icpc -S -O3 fverbose-asm -masm=intel  -std=c++17 -march=skylake-avx512 -mavx512f -falign-functions=32 GMS_config.h GMS_cephes_sin_cos.h GMS_simd_cephes_sin.h unit_test_mm512_cephes_sin_ps.cpp
...
void unit_test_mm512_cephes_sin_ps_1st_cycle();
...
void unit_test_mm512_cephes_sin_ps_1st_cycle()
{
    ...
    const float in_ref_5{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_6{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_7{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_8{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_9{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_10{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_11{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_12{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_13{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_14{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_15{sin_rand_in.operator()(rand_uni_gen)};
    const float in_ref_16{sin_rand_in.operator()(rand_uni_gen)};
    __m512    in_vref_vec{_mm512_setr_ps(in_ref_1,in_ref_2,in_ref_3,in_ref_4,in_ref_5,in_ref_6,in_ref_7,in_ref_8,
                                         in_ref_9,in_ref_10,in_ref_11,in_ref_12,in_ref_13,in_ref_14,in_ref_15,in_ref_16)};
    ...
    printf("[UNIT-TEST]: -- Calling reference implementation: ceph_sinf.\n");
    printf("[UNIT-TEST]: Inputs are: ref_1=%.7f,ref_2=%.7f,ref_3=%.7f,ref_4=%.7f\n",in_ref_1,in_ref_2,in_ref_3,in_ref_4);
    printf("[UNIT-TEST]: Inputs are: ref_5=%.7f,ref_6=%.7f,ref_7=%.7f,ref_8=%.7f\n",in_ref_5,in_ref_6,in_ref_7,in_ref_8);
    printf("[UNIT-TEST]: Inputs are: ref_9=%.7f,ref_10=%.7f,ref_11=%.7f,ref_12=%.7f\n",in_ref_9,in_ref_10,in_ref_11,in_ref_12);
    printf("[UNIT-TEST]: Inputs are: ref_13=%.7f,ref_14=%.7f,ref_15=%.7f,ref_16=%.7f\n",in_ref_13,in_ref_14,in_ref_15,in_ref_16);
    sca_expected_ref_1 = ceph_sinf(in_ref_1);
    sca_expected_ref_2 = ceph_sinf(in_ref_2);
    sca_expected_ref_3 = ceph_sinf(in_ref_3);
    sca_expected_ref_4 = ceph_sinf(in_ref_4);
    sca_expected_ref_5 = ceph_sinf(in_ref_5);
    sca_expected_ref_6 = ceph_sinf(in_ref_6);
    sca_expected_ref_7 = ceph_sinf(in_ref_7);
    sca_expected_ref_8 = ceph_sinf(in_ref_8);
    sca_expected_ref_9 = ceph_sinf(in_ref_9);
    sca_expected_ref_10 = ceph_sinf(in_ref_10);
    sca_expected_ref_11 = ceph_sinf(in_ref_11);
    sca_expected_ref_12 = ceph_sinf(in_ref_12);
    sca_expected_ref_13 = ceph_sinf(in_ref_13);
    sca_expected_ref_14 = ceph_sinf(in_ref_14);
    sca_expected_ref_15 = ceph_sinf(in_ref_15);
    sca_expected_ref_16 = ceph_sinf(in_ref_16);
    printf("[UNIT-TEST]: -- Calling AVX512 implementation: _mm512_ceph_sinf_ps.\n");
    ...
    //__asm__("int3");
    vec_actual_vref = _mm512_ceph_sinf_ps(in_vref_vec);
    ...
    if(!ref_5_approx_equality) 
       {std::printf("[UNIT-TEST #5]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_5,p_vref_out[4],sca_expected_ref_5-p_vref_out[4]);}
    if(!ref_6_approx_equality) 
       {std::printf("[UNIT-TEST #6]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_6,p_vref_out[5],sca_expected_ref_6-p_vref_out[5]);}
    if(!ref_7_approx_equality) 
       {std::printf("[UNIT-TEST #7]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_7,p_vref_out[6],sca_expected_ref_7-p_vref_out[6]);}
    if(!ref_8_approx_equality) 
       {std::printf("[UNIT-TEST #8]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_8,p_vref_out[7],sca_expected_ref_8-p_vref_out[7]);}
    if(!ref_9_approx_equality) 
       {std::printf("[UNIT-TEST #9]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_9,p_vref_out[8],sca_expected_ref_9-p_vref_out[8]);}
    if(!ref_10_approx_equality) 
       {std::printf("[UNIT-TEST #10]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_10,p_vref_out[9],sca_expected_ref_10-p_vref_out[9]);}
    if(!ref_11_approx_equality) 
       {std::printf("[UNIT-TEST #11]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_11,p_vref_out[10],sca_expected_ref_11-p_vref_out[10]);}
    if(!ref_12_approx_equality) 
       {std::printf("[UNIT-TEST #12]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_12,p_vref_out[11],sca_expected_ref_12-p_vref_out[11]);}
    if(!ref_13_approx_equality) 
       {std::printf("[UNIT-TEST #13]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_13,p_vref_out[12],sca_expected_ref_13-p_vref_out[12]);}
    if(!ref_14_approx_equality) 
       {std::printf("[UNIT-TEST #14]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_14,p_vref_out[13],sca_expected_ref_14-p_vref_out[13]);}
    if(!ref_15_approx_equality) 
       {std::printf("[UNIT-TEST #15]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_15,p_vref_out[14],sca_expected_ref_15-p_vref_out[14]);}
    if(!ref_16_approx_equality) 
       {std::printf("[UNIT-TEST #16]-- **FAILED**, sca=%.7f,vec=%.7f,delta=%.7f\n",sca_expected_ref_16,p_vref_out[15],sca_expected_ref_16-p_vref_out[15]);}
     printf("[UNIT_TEST]: function=%s -- **END**\n\n", __PRETTY_FUNCTION__);
}

__attribute__((hot))
__attribute__((noinline))
void unit_test_mm512_cephes_sin_ps_8191_cycles();
...
int main()
{
    unit_test_mm512_cephes_sin_ps_1st_cycle();
    unit_test_mm512_cephes_sin_ps_8191_cycles();
    return 0;
}