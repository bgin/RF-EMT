...
/*
   icpc -o unit_test_add_vec_xmm4r4_u -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5 -fopenmp \
   GMS_config.h GMS_malloc.h GMS_add_vec_xmm4r4.h GMS_add_vec_xmm4r4.cpp unit_test_add_vec_xmm4r4_u.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -fopenmp -falign-functions=32 GMS_config.h GMS_malloc.h GMS_add_vec_xmm4r4.h GMS_add_vec_xmm4r4.cpp unit_test_add_vec_xmm4r4_u.cpp
...
void unit_test_vadd_f32_2x128v_u16x_u();
...
void unit_test_vadd_f32_2x128v_u16x_u()
{
    ...
    std::string fname1 = "UNIT-TEST_uni_distro_vadd_f32_2x128v_u16x_u_" + std::to_string(funi_distro_idx++)+".csv";
    std::string fname2 = "UNIT-TEST_norm_distro_vadd_f32_2x128v_u16x_u_" + std::to_string(fnorm_distr_idx++)+".csv";
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            vec_z = reinterpret_cast<float * __restrict__>(std::malloc(nbytes));
            ...
            printf("[UNIT-TEST]: -- START: vadd_f32_2x128v_u16x_u\n");
            start   = __rdtscp(&tsc_aux);
            std::int32_t call_state = gms::math::vadd_f32_2x128v_u16x_u(vec_x,vec_y,vec_z,nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: vadd_f32_2x512v_u16x_u -- TSC=%llu,TSC_AUX=%d,call_state=%d\n",elapsed_tsc,tsc_aux,call_state);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
                 if(vec_z_neq_0) std::free(vec_z);
            ...
            if(vec_x_neq_0) std::free(vec_x);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: vadd_f32_2x128v_u16x_u\n");
        }
        break;
        case 1 :
        {
            ...
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: vadd_f32_2x128v_u16x_u() -- TSC=%llu,TSC_AUX=%d,call_state=%d\n",elapsed_tsc,tsc_aux,call_state);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_vadd_f32_2x128v_u10x_u();
...
void unit_test_vadd_f32_2x128v_u10x_u()
{
    ...
    constexpr std::size_t nbytes{reinterpret_cast<std::size_t>(sizeof(float)*nelems)};
    constexpr unsigned long long RDTSCP_LAT{42ull};
    std::string fname1 = "UNIT-TEST_uni_distro_vadd_f32_2x128v_u10x_u_" + std::to_string(funi_distro_idx++)+".csv";
    std::string fname2 = "UNIT-TEST_norm_distro_vadd_f32_2x128v_u10x_u_" + std::to_string(fnorm_distr_idx++)+".csv";
    float * __restrict__ vec_x{NULL};
    float * __restrict__ vec_y{NULL};
    float * __restrict__ vec_z{NULL};
    FILE  * __restrict__ fp{NULL};
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: vadd_f32_2x128v_u10x_u\n");
            start   = __rdtscp(&tsc_aux);
            std::int32_t call_state = gms::math::vadd_f32_2x128v_u10x_u(vec_x,vec_y,vec_z,nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: vadd_f32_2x128v_u10x_u -- TSC=%llu,TSC_AUX=%d,call_state=%d\n",elapsed_tsc,tsc_aux,call_state);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(vec_x_neq_0) gms_mm_free(vec_x);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: vadd_f32_2x128v_u10x_u\n");
        }
        break;
        case 1 :
        {
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_vadd_f32_2x128v_u6x_u();
...
void unit_test_vadd_f32_2x128v_u6x_u()
{
    ...
    constexpr std::size_t nbytes{reinterpret_cast<std::size_t>(sizeof(float)*nelems)};
    constexpr unsigned long long RDTSCP_LAT{42ull};
    std::string fname1 = "UNIT-TEST_uni_distro_vadd_f32_2x128v_u6x_u_" + std::to_string(funi_distro_idx++)+".csv";
    std::string fname2 = "UNIT-TEST_norm_distro_vadd_f32_2x128v_u6x_u_" + std::to_string(fnorm_distr_idx++)+".csv";
    float * __restrict__ vec_x{NULL};
    float * __restrict__ vec_y{NULL};
    float * __restrict__ vec_z{NULL};
    FILE  * __restrict__ fp{NULL};
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: vadd_f32_2x128v_u6x_u\n");
            start   = __rdtscp(&tsc_aux);
            std::int32_t call_state = gms::math::vadd_f32_2x128v_u6x_u(vec_x,vec_y,vec_z,nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: vadd_f32_2x128v_u6x_u -- TSC=%llu,TSC_AUX=%d,call_state=%d\n",elapsed_tsc,tsc_aux,call_state);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(vec_x_neq_0) std::free(vec_x);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: vadd_f32_2x128v_u6x_u\n");
        }
        break;
        case 1 :
        {
...
int main()
{  
   unit_test_vadd_f32_2x128v_u10x_u();
   unit_test_vadd_f32_2x128v_u10x_u();
   unit_test_vadd_f32_2x128v_u6x_u();
   return 0;
}