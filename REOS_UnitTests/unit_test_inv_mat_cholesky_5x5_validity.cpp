...
/*
   icpc -o unit_test_matrix_inv_cholesky_5x5_validity -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_malloc.h GMS_matrix_inv_cholesky.h GMS_matrix_inv_cholesky.cpp unit_test_mat_inv_cholesky_defines.h unit_test_matrix_inv_cholesky_5x5_validity.cpp 
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 GMS_config.h GMS_config.h GMS_malloc.h GMS_matrix_inv_cholesky.h GMS_matrix_inv_cholesky.cpp unit_test_mat_inv_cholesky_defines.h unit_test_matrix_inv_cholesky_5x5_validity.cpp 
*/
...
void unit_test_matrix_inv_cholesky_5x5_validity();

void unit_test_matrix_inv_cholesky_5x5_validity()
{
    using namespace gms::math;
    constexpr int32_t nelems{400};
    ...
    printf("[UNIT-TEST]: -- START: mat_inv_cholesky_5x5_16xf32<true,true> vs. matrix_inv_cholesky_5x5 (Validity).\n");
    seed_d = std::clock();
    ...
    switch (which)
    {
        case 0 : 
        {
            ...
            __ATTR_ALIGN__(64) __m512 matBRe[5][5];
            __ATTR_ALIGN__(64) __m512 matBIm[5][5];
            __ATTR_ALIGN__(64) __m512 matInvBRe[5][5];
            __ATTR_ALIGN__(64) __m512 matInvBIm[5][5];
            __ATTR_ALIGN__(64) __m512 matInvBReRef[5][5];
            __ATTR_ALIGN__(64) __m512 matInvBImRef[5][5];
             seed_re = __rdtsc();
            ...
            for(std::int32_t i = 0;i != MAT_SQR_SIZE_5; ++i)
            {   
                const std::int32_t outer_idx = i*MAT_SQR_SIZE_5;
                for(std::int32_t j = 0;j != MAT_SQR_SIZE_5; ++j)  
                {
            ...
            printf_ret = printf("[UNIT-TEST]: -- START: mat_inv_cholesky_5x5_16xf32<true,true>.\n");
            gms::math::mat_inv_cholesky_5x5_16xf32<false,true>(matBRe,matBIm,
                                                               matInvBRe,matInvBIm);                                                  
            ptr_invBRe = reinterpret_cast<const float * __restrict>(&matInvBRe);
            ptr_invBIm = reinterpret_cast<const float * __restrict>(&matInvBIm);
            printf_ret = printf("[UNIT-TEST]: -- START: matrix_inv_cholesky_5x5(***FlexRAN(Reference)***).\n");
            matrix_inv_cholesky_5x5(matBRe,matBIm,matInvBReRef,matInvBImRef);
            ...
            for(int32_t __i{0}; __i != nelems; ++__i)
            {
                ...
                if(detect_subnormal(bre))     { std::printf("[UNIT-TEST:] -- (RF-EMT)  ***SUBNORMAL*** in I-ch -- %.17f\n",bre);}
                if(detect_subnormal(bim))     { std::printf("[UNIT-TEST:] -- (RF-EMT)  ***SUBNORMAL*** in Q-ch -- %.17f\n",bim);}
                if(detect_subnormal(bre_ref)) { std::printf("[UNIT-TEST:] -- (FlexRAN) ***SUBNORMAL*** in I-ch -- %.17f\n",bre_ref);}
                if(detect_subnormal(bim_ref)) { std::printf("[UNIT-TEST:] -- (FlexRAN) ***SUBNORMAL*** in Q-ch -- %.17f\n",bim_ref);}
            ...
            if(!bim_err_count) {printf_ret = printf("[UNIT_TEST:] -- Validity Test: (Q-channel): ***PASSED***\n");}
             ...
             printf_ret = printf("[UNIT-TEST]: -- END: mat_inv_cholesky_5x5_16xf32<true,true> vs matrix_inv_cholesky_5x5 (Validity).\n");
        ...
        case 1 : 
        {
            ...
            printf("[UNIT-TEST]: -- START: mat_inv_cholesky_5x5_16xf32<true,true> ***RF-EMT*** .\n");
            gms::math::mat_inv_cholesky_5x5_16xf32<false,true>(matBRe,matBIm,
                                                               matInvBRe,matInvBIm);
            ptr_invBRe = reinterpret_cast<const float * __restrict>(&matInvBRe);
            ptr_invBIm = reinterpret_cast<const float * __restrict>(&matInvBIm);
            ...
            printf_ret = printf("[UNIT-TEST]: -- END: mat_inv_cholesky_5x5_16xf32<false,true> vs matrix_inv_cholesky_5x5 (Validity).\n");
        }
        break;
        case 2 : 
        {
            ...
            printf_ret = printf("[UNIT-TEST]: -- START: mat_inv_cholesky_5x5_16xf32<true,true> ***RF-EMT*** .\n");
            gms::math::mat_inv_cholesky_5x5_16xf32<false,true>(matBRe,matBIm,
                                                               matInvBRe,matInvBIm);
            ptr_invBRe = reinterpret_cast<const float * __restrict>(&matInvBRe);
            ptr_invBIm = reinterpret_cast<const float * __restrict>(&matInvBIm);
...
int main()
{   
    unit_test_matrix_inv_cholesky_5x5_validity();
    return 0;
}
