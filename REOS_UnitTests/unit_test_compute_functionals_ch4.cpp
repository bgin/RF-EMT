...
void unit_test_compute_functional_4_1_gauss_Q()
{
    ...
    for(std::int32_t ii{1}; ii<4; ++ii)
    {
        ...
        for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
        {
            print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
        }
    }
...
void unit_test_compute_functional_4_2_gauss_Q()
{
    ...
    integrator_payload.tmp_work1 = &tmp_work[0];
    integrator_payload.inf   = &inf[0];
    integrator_payload.irule = &irule[0];
    integrator_payload.epsabs = &epsabs[0];
    integrator_payload.epsrel = &epsrel[0];
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_6_gauss_Q();
...
void unit_test_compute_functional_4_6_gauss_Q()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_6_gauss_Q_quad_iface;
    integrator_payload.func_args_payload = &func_args[0];
    ...
    for(std::int32_t ii{1}; ii<4; ++ii) 
    {
        ...
        if(p_payload->which_integrator!=2)
        {
           for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
           {
               print_retv = print_double("Gauss-Q (f:4.6)",p_payload->functional[i],0);
           }
           for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
           {
               print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_7_gauss_Q();
...
void unit_test_compute_functional_4_7_gauss_Q()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_7_gauss_Q_quad_iface;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.tmp_work3 = &tmp_work3[0];
    ...
    for(std::int32_t ii{1}; ii<5; ++ii) 
    {
        ...
        if(p_payload->which_integrator!=2)
        {
           for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
           {
               print_retv = print_double("Gauss-Q (f:4.7)",p_payload->functional[i],0);
           }
           for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
           {
               print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_8_gauss_Q();
...
void unit_test_compute_functional_4_8_gauss_Q()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_8_gauss_Q_quad_iface;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.inf   = &inf[0];
    ...
    for(std::int32_t ii{1}; ii<5; ++ii) 
    {
        ...
        if(p_payload->which_integrator!=2)
        {
           for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
           {
               print_retv = print_double("Gauss-Q (f:4.8)",p_payload->functional[i],0);
           }
           for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
           {
               print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_10_marcum_Q();
...
void unit_test_compute_functional_4_10_marcum_Q()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_10_marcum_Q_quad_iface;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.inf   = &inf[0];
    ...
    integrator_payload.epsrel = &epsrel[0];
    integrator_payload.abser  = &abser[0];
    integrator_payload.functional = &marcum_Q_functional[0];
    integrator_payload.neval = &neval[0];
    integrator_payload.ier = &ier[0];
    integrator_payload.last = &last[0];
    integrator_payload.crude_tsc_measurement = &crude_tsc_results[0];
    integrator_payload.rand_lo1 = +0.1;
    integrator_payload.rand_hi1 = +10.0;
    integrator_payload.rand_lo2 = +0.0;
    integrator_payload.rand_hi2 = +2.5;
    integrator_payload.rand_lo3 = +11.0;
    integrator_payload.n_func_vals = n_func_args;
    integrator_payload.randomly_generate_inputs = true;
    //integrator_payload.which_integrator = 3;
    gms::fading_channel::quadpack_integrator_payload_t * __restrict__ p_payload = &integrator_payload;
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_16_marcum_Q();
...
void unit_test_compute_functional_4_16_marcum_Q()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_16_marcum_Q_quad_iface;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.inf   = &inf[0];
    ...
    for(std::int32_t ii{1}; ii<5; ++ii) 
    {
        ...
        if(integrator_payload.which_integrator!=2)
        {
            for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
            {
                print_retv = print_double("Marcum-Q (f:4.16)",p_payload->functional[i],0);
            }
            for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
            {
                print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_20_marcum_Q_lo();
...
void unit_test_compute_functional_4_20_marcum_Q_lo()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_20_marcum_Q_quad_iface_lo;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.inf   = &inf[0];
    ...
    for(std::int32_t ii{1}; ii<5; ++ii) 
    {
        ...
        if(integrator_payload.which_integrator!=2)
        {
            for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
            {
                print_retv = print_double("Marcum-Q (f:4.20-lo)",p_payload->functional[i],0);
            }
            for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
            {
                print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_20_marcum_Q_hi();
...
void unit_test_compute_functional_4_20_marcum_Q_hi()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_20_marcum_Q_quad_iface_hi;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.inf   = &inf[0];
    ...
    for(std::int32_t ii{1}; ii<5; ++ii) 
    {
        ...
        if(integrator_payload.which_integrator!=2)
        {
            for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
            {
                print_retv = print_double("Marcum-Q (f:4.20-hi)",p_payload->functional[i],0);
            }
            for(std::int32_t i{0}; i < p_payload->n_func_vals; ++i) 
            {
                print_retv = std::printf("Iter=%d,abser=%.17f,neval=%d,ier=%d,last=%d,total_tsc=%llu,ret=%d\n",i,abser[i],neval[i],ier[i],last[i],crude_tsc_results[i],integrator_ret);
...
__attribute__((hot))
__attribute__((aligned(32)))
void unit_test_compute_functional_4_32_marcum_m_Q();
...
void unit_test_compute_functional_4_32_marcum_m_Q()
{
    ...
    irule[0]  = 4;
    gms::fading_channel::quadpack_integrator_payload_t integrator_payload;
    integrator_payload.integrand = &gms::fading_channel::integrand_4_32_marcum_Q_m_quad_iface;
    integrator_payload.func_args_payload = &func_args[0];
    integrator_payload.tmp_work1 = &tmp_work1[0];
    integrator_payload.tmp_work2 = &tmp_work2[0];
    integrator_payload.inf   = &inf[0];
    ...
    integrator_payload.rand_hi1 = +10.0;
    integrator_payload.rand_lo2 = +0.0;
    integrator_payload.rand_hi2 = +5.5;
    integrator_payload.rand_lo3 = 21.12;
