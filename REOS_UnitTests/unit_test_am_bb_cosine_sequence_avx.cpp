...
{
     std::size_t i,j;
     for(i = 0ull; i != ROUND_TO_EIGHT(sig_len,8ull); i += 8ull) 
     {
         __m256 xmm1 = _mm256_load_ps(&sig_1[i]);
         __m256 xmm2 = _mm256_load_ps(&sig_2[i]);
         __m256 sum0 = _mm256_add_ps(xmm1,xmm2);
         __m256 xmm3 = _mm256_load_ps(&sig_3[i]);
         __m256 xmm4 = _mm256_load_ps(&sig_4[i]);
         __m256 sum1 = _mm256_add_ps(xmm3,xmm4);
         __m256 sum2 = _mm256_add_ps(sum0,sum1);
         _mm256_store_ps(&sig_vsumm[i],sum2);
     }
...
__attribute__((hot))
__attribute__((noinline))
void unit_test_am_bb_cosine_sequence_vsummation_correctness();
...
void unit_test_am_bb_cosine_sequence_vsummation_correctness()
{
    ...
    // auto rand_I_n_samples{std::bind(std::uniform_int_distribution<std::size_t>(124ull,8192ull),std::mt19937(seed_I_n_samples))};
     ...
     std::printf("[UNIT-TEST:] -- Creating gnuplot plotting command file.\n\n");
     am_bb_cos_signal_sequence_t::create_sequence_plot(cos_sequence.m_nsamples,&cos_sequence.m_vsummed_sequence.operator[](0),nullptr,
                                              "am_bb_cosine_sequence_avx_u6x_correctness_test_7_","AM-BB_Cosine_Signals_Sequence",false);
     am_bb_cos_signal_sequence_t::create_sequence_plot(I_n_samples,&sig_vsum_reference.m_data[0],nullptr,
                                               "am_bb_cosine_signal_reference_vsumm_test_7_","AM-BB_Cosine_Signals_Sequence",false);
...
int main()
{
    ...
    unit_test_am_bb_cosine_sequence_vsummation_correctness();
