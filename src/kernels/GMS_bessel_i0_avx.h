...
#ifndef __GMS_BESSEL_I0_AVX_H__
#define __GMS_BESSEL_I0_AVX_H__ 100820261054
...
namespace file_info 
{
     ...
     static const unsigned int GMS_BESSEL_I0_AVX_MAJOR = 1;
     static const unsigned int GMS_BESSEL_I0_AVX_MINOR = 1;
     static const unsigned int GMS_BESSEL_I0_AVX_MICRO = 0;
     static const unsigned int GMS_BESSEL_I0_AVX_FULLVER =
       1000U*GMS_BESSEL_I0_AVX_MAJOR+100U*GMS_BESSEL_I0_AVX_MINOR+
       10U*GMS_BESSEL_I0_AVX_MICRO;
     static const char GMS_BESSEL_I0_AVX_CREATION_DATE[] = "10-08-2026 10:55AM +00200 (MON 10 AUG 2026 GMT+2)";
     static const char GMS_BESSEL_I0_AVX_BUILD_DATE[]    = __DATE__; 
     static const char GMS_BESSEL_I0_AVX_BUILD_TIME[]    = __TIME__;
     static const char GMS_BESSEL_I0_AVX_SYNOPSIS[]      = "Manual vectorization of the Bessel-I0 scalar function. Authors: \
...
static inline 
__m256d
bessel_i0_le15_avx_pd(const __m256d arg)
{
  __ATTR_ALIGN__(32) const __m256d p[15] = 
  {
    _mm256_set1_pd(-5.2487866627945699800E-18),
  ...
  __ATTR_ALIGN__(16) const __m256d q[5] = 
  {
    _mm256_set1_pd(-3.7277560179962773046E+03), 
  ...
  const __m256d C225{_mm256_set1_pd(225.0)};
  __m256d sump;
  __m256d sumq;
  __m256d result;
  //__m256d x  = _mm256_abs_pd(arg);
  __m256d xx = _mm256_mul_pd(arg,arg);
  sump = p[0];
  ...
  xx   = _mm256_sub_pd(xx,C225);
  /*
     double sumq = ((((xx + q[0]) * xx + q[1]) * xx + q[2]) * xx + q[3]) * xx + q[4];
  */	
  sumq   = _mm256_fmadd_pd(_mm256_fmadd_pd(_mm256_fmadd_pd(_mm256_fmadd_pd(_mm256_add_pd(xx,q[0]),xx,q[1]),xx,q[2]),xx,q[3]),xx,q[4]);    
  result = _mm256_div_pd(sump,sumq);
  return (result);
}
...
static inline 
__m256d
bessel_i0_ge15_avx_pd(const __m256d arg)
{
  __ATTR_ALIGN__(32) const __m256d pp[8] = 
  {
    _mm256_set1_pd(-3.9843750000000000000E-01), 
  ...
  __ATTR_ALIGN__(32) const __m256d qq[7] =
  {
    _mm256_set1_pd(-3.1446690275135491500E+01), 
  ...
  const __m256d one   = _mm256_set1_pd(1.0e+00);
  const __m256d C15   = _mm256_set1_pd(15.0e+00);
  const __m256d exp40 = _mm256_set1_pd(2.353852668370199854E+17);
  const __m256d C40   = _mm256_set1_pd(40.0e+00);
  const __m256d xmax  = _mm256_set1_pd(91.9E+00);
  __m256d       xx    = _mm256_sub_pd(_mm256_div_pd(one,arg),rec15);
  ...
  sumq                = c5;
  result              = _mm256_div_pd(sump,sumq);
  const __mmask8 m    = _mm256_cmp_pd_mask(arg,_mm256_sub_pd(xmax,C15),_CMP_LE_OQ);
  a                   = _mm256_mask_blend_pd(m,_mm256_exp_pd(_mm256_sub_pd(arg,C40)),
					                                             _mm256_exp_pd(arg));
  b                   = _mm256_mask_blend_pd(m,exp40,one);
  const __m256d tmp   = _mm256_fmsub_pd(result,a,_mm256_mul_pd(pp[0],a));
  result              = _mm256_mul_pd(_mm256_div_pd(tmp,_mm256_sqrt_pd(arg)),b);
  return (result);
}
