...
gms::radiolocation
::am_bb_cmplx_cos_signal_fp16_t
::am_bb_cmplx_cos_signal_fp16_t(const std::size_t nsamples,
...
gms::radiolocation
::am_bb_cmplx_cos_signal_fp16_t
::am_bb_cmplx_cos_signal_fp16_t(am_bb_cmplx_cos_signal_fp16_t &&other) noexcept(true)
:
...
gms::radiolocation
::am_bb_cmplx_cos_signal_fp16_t
::am_bb_cmplx_cos_signal_fp16_t(const am_bb_cmplx_cos_signal_fp16_t &other) noexcept(false)
:
...
gms::radiolocation
::am_bb_cmplx_cos_signal_fp16_t
::operator=(am_bb_cmplx_cos_signal_fp16_t &&other) noexcept(true)
{
                          ...
                          const std::uint32_t n_K)
{
      ...
      if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
       ...
       for(std::uint32_t __t{0}; __t != n_T; ++__t) 
       {
            ...
            std::complex<half> tmp_fp16(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
            this->m_sig_samples.m_data[__t] = tmp_fp16;/*half_cast<std::complex<half>>(sum);*/
        }
        return (0);
}
...
gms::radiolocation
::am_bb_cmplx_cos_signal_fp16_t
::create_noisy_signal_user_data(am_bb_cmplx_cos_signal_fp16_pdf_params_t & pdf_params,
                                const float scale,
                                ...
                                const std::uint32_t n_K)
{
      ...
      for(std::uint32_t __t{0}; __t != n_T; ++__t) 
      {
            ...
            std::complex<half> tmp_fp16(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
            this->m_sig_samples.m_data[__t] = tmp_fp16;
        }
        return (0);
}
...
::am_bb_cmplx_cos_signal_fp16_t
::create_noisy_signal_user_data(am_bb_cmplx_cos_signal_fp16_pdf_params_t & pdf_params,
                                am_bb_cmplx_cos_signal_fp16_rand_distr rd_enum,
                                const float scale,
                                const float * __restrict__ sym_in, // size of m_nsamples*m_nK values [0,1]
                                const std::uint32_t n_T,
                                const std::uint32_t n_K)
{
     ...
     if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
      ...
      switch (rd_enum)
      {
        case am_bb_cmplx_cos_signal_fp16_rand_distr::uniform : 
        {
              tsc = __rdtsc();
        ...
        break;
        case am_bb_cmplx_cos_signal_fp16_rand_distr::normal : 
        {
              tsc = __rdtsc();
         ...
         break;
         case am_bb_cmplx_cos_signal_fp16_rand_distr::cauchy : 
         {
                tsc = __rdtsc();
         ...
         break;
         case am_bb_cmplx_cos_signal_fp16_rand_distr::log_norm : 
         {
               tsc = __rdtsc();
         ...
         break;
         case am_bb_cmplx_cos_signal_fp16_rand_distr::expo_gamma : 
         {
               tsc = __rdtsc();
          ...
          break;
          case am_bb_cmplx_cos_signal_fp16_rand_distr::weibull : 
          {
                 tsc = __rdtsc();
          ...
          break;
          case am_bb_cmplx_cos_signal_fp16_rand_distr::gamma : 
          {
                 tsc = __rdtsc();
...
::operator<<(std::ostream &os,
             gms::radiolocation::am_bb_cmplx_cos_signal_fp16_t &rhs)->std::ostream &
{
