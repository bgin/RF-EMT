...
::am_bb_cmplx_cosinc_signal_t(const std::size_t nsamples,
...
gms::radiolocation
::am_bb_cmplx_cosinc_signal_t
::am_bb_cmplx_cosinc_signal_t(am_bb_cmplx_cosinc_signal_t &&other) noexcept(true)
:
...
gms::radiolocation
::am_bb_cmplx_cosinc_signal_t
::operator=(const am_bb_cmplx_cosinc_signal_t &other) noexcept(false)
{
...
gms::radiolocation
::am_bb_cmplx_cosinc_signal_t
::operator=(am_bb_cmplx_cosinc_signal_t &&other) noexcept(true)
{
    if(__builtin_expect(this==&other,0)) { return (*this);}
      ...
      if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
       ...
       for(std::uint32_t __t{0}; __t != n_T; ++__t) 
       {
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                 ...
                 sum += cosinc_sample(arg,invT)*(C141421356237309504880168872421*csym);
            }
...
gms::radiolocation
::am_bb_cmplx_cosinc_signal_t
::create_signal_user_data_v2(const float * __restrict__ sym_in, // size of m_nsamples*m_nK
                             const std::uint32_t n_n,
                             const std::uint32_t n_T,
                             const std::uint32_t n_K)
{
      if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
       ...
       const float T{static_cast<float>(this->m_nsamples)};
       //const float n{static_cast<float>(n_n)};
       constexpr float C141421356237309504880168872421{1.41421356237309504880168872421f};
       const float invT{this->m_P/T};
       std::complex<float> sum;
       for(std::uint32_t __n{1}; __n != n_n; ++__n) 
       {
           const float n{static_cast<float>(__n)};
           for(std::uint32_t __t{0}; __t != n_T; ++__t) 
           {
               const float t{static_cast<float>(__t)};
               sum = (0.0f,0.0f);
               for(std::uint32_t __k{0}; __k != n_K; ++__k) 
               {
                    ...
                    const float re_im{1.0f-(2.0f*sym)};
                    const std::complex<float> csym{re_im,re_im};
                    sum += cosinc_sample_v2(arg,n,invT)*(C141421356237309504880168872421*csym);
               }
               this->m_sig_samples.m_data[__t] = sum;
           }
       }
      ...
      if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
        ...
        for(__i = 0; __i != ROUND_TO_FOUR(n_T,4); __i += 4u)
        {
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                ...
                const float re_im{1.0f-(2.0f*sym)};
                const std::complex<float> csym{re_im,re_im};
                sum0 += cosinc_sample(arg,invT)*(C141421356237309504880168872421*csym);
            }
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                ...
                const float re_im{1.0f-(2.0f*sym)};
                const std::complex<float> csym{re_im,re_im};
                sum1 += cosinc_sample(arg,invT)*(C141421356237309504880168872421*csym);
            }
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                ...
                const float re_im{1.0f-(2.0f*sym)};
                const std::complex<float> csym{re_im,re_im};
                sum2 += cosinc_sample(arg,invT)*(C141421356237309504880168872421*csym);
            }
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                ...
                const float re_im{1.0f-(2.0f*sym)};
                const std::complex<float> csym{re_im,re_im};
                sum3 += cosinc_sample(arg,invT)*(C141421356237309504880168872421*csym);
            }
...
gms::radiolocation
::am_bb_cmplx_cosinc_signal_t
::create_noisy_signal_user_data(am_bb_cmplx_cosinc_signal_pdf_params_t & pdf_params,
                                const float scale,
                                ...
                                const std::uint32_t n_K)
{
      ...
      for(std::uint32_t __t{0}; __t != n_T; ++__t) 
      {
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                 ...
                 const float re_im{1.0f-(2.0f*sym)};
                 const std::complex<float> csym{re_im,re_im};
                 sum += cosinc_sample_noisy(arg,invT,scale,uni_rand)*(C141421356237309504880168872421*csym);
            }
            this->m_sig_samples.m_data[__t] = sum;
        }
        return (0);
                                ...
                                const std::uint32_t n_K)
{
     if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
      ...
      switch (rd_enum)
      {
        case am_bb_cmplx_cosinc_signal_rand_distr::uniform : 
              tsc = __rdtsc();
              ...
              for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
              {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          sum += cosinc_sample_noisy(arg,invT,scale,uni_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
        break;
        case am_bb_cmplx_cosinc_signal_rand_distr::normal : 
              tsc = __rdtsc();
              ...
              for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
              {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += cosinc_sample_noisy(arg,invT,scale,normal_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
         break;
         case am_bb_cmplx_cosinc_signal_rand_distr::cauchy : 
                tsc = __rdtsc();
                ...
                for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += cosinc_sample_noisy(arg,invT,scale,cauchy_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
         break;
         case am_bb_cmplx_cosinc_signal_rand_distr::log_norm : 
               tsc = __rdtsc();
               auto lognorm_rand{std::bind(std::lognormal_distribution<float>(pdf_params.log_norm_m_r,pdf_params.log_norm_s_r),
                ...
                for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += cosinc_sample_noisy(arg,invT,scale,lognorm_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
         break;
         case am_bb_cmplx_cosinc_signal_rand_distr::expo_gamma : 
               tsc = __rdtsc();
               auto expo_rand{std::bind(std::exponential_distribution<float>(pdf_params.expo_gamma_r),
                ...
                for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += cosinc_sample_noisy(arg,invT,scale,expo_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
          break;
          case am_bb_cmplx_cosinc_signal_rand_distr::weibull : 
                 tsc = __rdtsc();
                 ...
                 for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                 {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += cosinc_sample_noisy(arg,invT,scale,weibull_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
          break;
          case am_bb_cmplx_cosinc_signal_rand_distr::gamma : 
                 tsc = __rdtsc();
                 ...
                 for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                 {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += cosinc_sample_noisy(arg,invT,scale,gamma_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     this->m_sig_samples.m_data[__t] = sum;
               }
           break;
...
::operator<<(std::ostream &os,
             gms::radiolocation::am_bb_cmplx_cosinc_signal_t &rhs)->std::ostream &
{
