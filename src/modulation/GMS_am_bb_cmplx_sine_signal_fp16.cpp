...
gms::radiolocation
::am_bb_cmplx_sin_signal_fp16_t
::am_bb_cmplx_sin_signal_fp16_t(const std::size_t nsamples,
...
gms::radiolocation
::am_bb_cmplx_sin_signal_fp16_t
::am_bb_cmplx_sin_signal_fp16_t(am_bb_cmplx_sin_signal_fp16_t &&other) noexcept(true)
:
...
gms::radiolocation
::am_bb_cmplx_sin_signal_fp16_t
::am_bb_cmplx_sin_signal_fp16_t(const am_bb_cmplx_sin_signal_fp16_t &other) noexcept(false)
:
...
gms::radiolocation
::am_bb_cmplx_sin_signal_fp16_t
::operator=(am_bb_cmplx_sin_signal_fp16_t &&other) noexcept(true)
{
      ...
      if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
        ...
        for(__i = 0; __i != ROUND_TO_FOUR(n_T,4); __i += 4u)
        {
            ...
            for(__j = __i; __j != n_T; ++__j) 
            {
                ...
                std::complex<half> tmp_rem(half_cast<half>(sum0.real()),half_cast<half>(sum0.imag()));
                this->m_sig_samples.m_data[__j] = tmp_rem;
            }
            return (0);
}
...
gms::radiolocation
::am_bb_cmplx_sin_signal_fp16_t
::create_noisy_signal_user_data(am_bb_cmplx_sin_signal_fp16_pdf_params_t & pdf_params,
                                const float scale,
                                ...
                                const std::uint32_t n_K)
{
      ...
      for(std::uint32_t __t{0}; __t != n_T; ++__t) 
      {
            ...
            for(std::uint32_t __k{0}; __k != n_K; ++__k) 
            {
                 ...
                 const float re_im{1.0f-(2.0f*sym)};
                 const std::complex<float> csym{re_im,re_im};
                 sum += sin_sample_noisy(arg,invT,scale,uni_rand)*(C141421356237309504880168872421*csym);
            }
...
::am_bb_cmplx_sin_signal_fp16_t
::create_noisy_signal_user_data(am_bb_cmplx_sin_signal_fp16_pdf_params_t & pdf_params,
                                am_bb_cmplx_sin_signal_fp16_rand_distr rd_enum,
                                const float scale,
                                const float * __restrict__ sym_in, // size of m_nsamples*m_nK values [0,1]
                                const std::uint32_t n_T,
                                const std::uint32_t n_K)
{
     ...
     if(__builtin_expect(static_cast<std::uint32_t>(this->m_nsamples)!=n_T,0) || 
      ...
      switch (rd_enum)
      {
        case am_bb_cmplx_sin_signal_fp16_rand_distr::uniform : 
        {
              tsc = __rdtsc();
              ...
              for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
              {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          sum += sin_sample_noisy(arg,invT,scale,uni_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
                     this->m_sig_samples.m_data[__t] = tmp_sum;
               }
        ...
        break;
        case am_bb_cmplx_sin_signal_fp16_rand_distr::normal : 
        {
              tsc = __rdtsc();
              ...
              for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
              {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += sin_sample_noisy(arg,invT,scale,normal_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
                     this->m_sig_samples.m_data[__t] = tmp_sum;
               }
         ...
         break;
         case am_bb_cmplx_sin_signal_fp16_rand_distr::cauchy : 
         {
                tsc = __rdtsc();
                ...
                for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += sin_sample_noisy(arg,invT,scale,cauchy_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
                     this->m_sig_samples.m_data[__t] = tmp_sum;
               }
         ...
         break;
         case am_bb_cmplx_sin_signal_fp16_rand_distr::log_norm : 
         {
               tsc = __rdtsc();
               auto lognorm_rand{std::bind(std::lognormal_distribution<float>(pdf_params.log_norm_m_r,pdf_params.log_norm_s_r),
                ...
                for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += sin_sample_noisy(arg,invT,scale,lognorm_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
                     this->m_sig_samples.m_data[__t] = tmp_sum;
               }
         ...
         break;
         case am_bb_cmplx_sin_signal_fp16_rand_distr::expo_gamma : 
         {
               tsc = __rdtsc();
               auto expo_rand{std::bind(std::exponential_distribution<float>(pdf_params.expo_gamma_r),
                ...
                for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += sin_sample_noisy(arg,invT,scale,expo_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
          ...
          break;
          case am_bb_cmplx_sin_signal_fp16_rand_distr::weibull : 
          {
                 tsc = __rdtsc();
                 ...
                 for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                 {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += sin_sample_noisy(arg,invT,scale,weibull_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
                     this->m_sig_samples.m_data[__t] = sum;
               }
          ...
          break;
          case am_bb_cmplx_sin_signal_fp16_rand_distr::gamma : 
          {
                 tsc = __rdtsc();
                 ...
                 for(std::uint32_t __t{0ull}; __t != n_T; ++__t) 
                 {
                     ...
                     for(std::uint32_t __k{0}; __k != n_K; ++__k) 
                     {
                          ...
                          const float re_im{1.0f-(2.0f*sym)};
                          const std::complex<float> c_sym{re_im,re_im};
                          sum += sin_sample_noisy(arg,invT,scale,gamma_rand)*(C141421356237309504880168872421*c_sym);
                     }
                     std::complex<half> tmp_sum(half_cast<half>(sum.real()),half_cast<half>(sum.imag()));
                     this->m_sig_samples.m_data[__t] = sum;
               }
...
::operator<<(std::ostream &os,
             gms::radiolocation::am_bb_cmplx_sin_signal_fp16_t &rhs)->std::ostream &
{
