...
#pragma omp threadprivate(rv_func_gamma_gen)
#pragma omp threadprivate(seed_func_gamma)
#pragma omp threadprivate(rv_func_q)
#pragma omp threadprivate(rv_func_q_gen)
#pragma omp threadprivate(seed_func_q)
#else
       thread_local std::uniform_real_distribution<double> rv_func_a;
       thread_local std::mt19937 rv_func_a_gen;
       thread_local std::uint64_t seed_func_a{};
...
#pragma omp threadprivate(seed_func_n)
    ...
    {
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
#pragma omp parallel for default(none) private(i,cpy_a,cpy_gamma,cpy_n,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_funcs_args_payload,p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
...
#pragma omp threadprivate(seed_func_m)
    ...
    {
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
#pragma omp parallel for default(none) private(i,cpy_a,cpy_gamma,cpy_m,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_funcs_args_payload,p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif   
...
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_a,cpy_mu,cpy_sigma,start,result,end) \
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg4d = cpy_sigma;
            start = gms::common::rdtsc_serialized_start();
            result = dqagi(p_integrand,rand_low5,p_inf[0],p_epsabs[0],p_epsrel[0],
...
::compute_functional_LNSh_Nakagami_m_chan_5_25(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    ...
    double  * __restrict__      p_tmp_work3                  = p_payload->tmp_work3; 
    double  * __restrict__      p_tmp_work4                  = p_payload->tmp_work4;
    double  * __restrict__      p_tmp_work5                  = p_payload->tmp_work5;
    double  * __restrict__      p_bound                      = p_payload->bound;
    std::int32_t * __restrict__ p_inf                        = p_payload->inf;
    std::int32_t * __restrict__ p_irule                      = p_payload->irule;
    double  *  __restrict__     p_epsabs                     = p_payload->epsabs;
...
#pragma omp threadprivate(seed_func_m)
    ...
    {
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_a,cpy_mu,cpy_sigma,cpy_m,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_tmp_work4,p_tmp_work5,p_funcs_args_payload, \
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_a,cpy_mu,cpy_sigma,cpy_m,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_tmp_work4,p_tmp_work5, \
               p_funcs_args_payload,p_integrand,p_epsabs,p_epsrel,p_abser,\
               rand_low5,p_inf,p_neval,p_ier,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
        for(i = 0; i<nfunc_vals; ++i)
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_a,cpy_mu,cpy_sigma,cpy_m,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_tmp_work4,p_tmp_work6,p_funcs_args_payload, \
               p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               rand_low5,rand_high5,p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_a,cpy_mu,cpy_sigma,cpy_m,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_tmp_work4,p_tmp_work6, \
               p_funcs_args_payload,p_integrand,p_epsabs,p_epsrel,p_abser,\
               rand_low5,p_inf,p_neval,p_ier,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
...
std::int32_t 
gms::fading_channel
::compute_functional_Rayleigh_LaplaceT_chan_5_39(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
    ...
    static   std::uniform_real_distribution<double> rv_func_psi;
    static   std::mt19937 rv_func_psi_gen;
    static   std::uint64_t seed_func_psi{};
    static   std::uniform_real_distribution<double> rv_func_gamma;
    static   std::mt19937 rv_func_gamma_gen;
    static   std::uint64_t seed_func_gamma{};
...
#pragma omp threadprivate(rv_func_psi)
#pragma omp threadprivate(rv_func_psi_gen)
#pragma omp threadprivate(seed_func_psi)
#pragma omp threadprivate(rv_func_gamma)
#pragma omp threadprivate(rv_func_gamma_gen)
#pragma omp threadprivate(seed_func_gamma)
...
#pragma omp threadprivate(seed_func_l)
    ...
    thread_local   std::uint64_t seed_func_l{};
       ...
       rv_func_psi = std::uniform_real_distribution<double>(rand_low2,rand_high2);
       ...
       for(std::int32_t i{0}; i<nfunc_vals; ++i) 
       {
           ...
           p_tmp_work1[i] = val_b;
           const double val_psi{rv_func_gamma.operator()(rv_func_psi_gen)};
           p_tmp_work2[i] = val_psi;
           const double val_gamma{rv_func_gamma.operator()(rv_func_gamma_gen)};
...
#pragma omp parallel for default(none) private(i,cpy_b,cpy_psi,cpy_gamma,cpy_l,start,result,end) \
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            cpy_psi = p_tmp_work2[i];
            p_funcs_args_payload[i].arg2d = cpy_psi;
...
#pragma omp parallel for default(none) private(i,cpy_b,cpy_psi,cpy_gamma,cpy_l,start,result,end) \
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg4d = cpy_l;
            start = gms::common::rdtsc_serialized_start();
            result = dqags(p_integrand,-3.141592653589793238462643383,+3.141592653589793238462643383,p_epsabs[0],p_epsrel[0],&p_abser[i],
                                        &p_neval[i],&p_ier[i],&p_funcs_args_payload[i]);
            end = gms::common::rdtsc_serialized_stop();
            p_functional[i] = 0.159154943091895335768883763*result;
            p_crude_tsc_start[i] = start;
...
#pragma omp parallel for default(none) private(i,cpy_b,cpy_psi,cpy_gamma,cpy_l,start,result,end) \
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg4d = cpy_l;
            start = gms::common::rdtsc_serialized_start();
            result = dqng(p_integrand,-3.141592653589793238462643383,+-3.141592653589793238462643383,p_epsabs[0],p_epsrel[0],&p_abser[i],
                                        &p_neval[i],&p_ier[i],&p_funcs_args_payload[i]);
            end = gms::common::rdtsc_serialized_stop();
            p_functional[i] = 0.159154943091895335768883763*result;
            p_crude_tsc_start[i] = start;
...
std::int32_t 
gms::fading_channel
::compute_functional_Hoyt_LaplaceT_chan_5_40(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
...
#pragma omp threadprivate(seed_func_l)
    ...
    thread_local   std::uint64_t seed_func_l{};
       ...
       seed_func_psi = __rdtsc();
       rv_func_psi_gen = std::mt19937(seed_func_psi);
       rv_func_q = std::uniform_real_distribution<double>(rand_low5,rand_high5);
       seed_func_q = __rdtsc();
       rv_func_q_gen = std::mt19937(seed_func_q);
       rv_func_gamma = std::uniform_real_distribution<double>(rand_low3,rand_high3);
       seed_func_gamma = __rdtsc();
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
        p_omp_loop_start[0] = gms::common::rdtsc_serialized_start();
#pragma omp parallel for default(none) private(i,cpy_b,cpy_psi,cpy_q,cpy_gamma,cpy_l,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work5,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg3d = cpy_q;
            cpy_gamma = p_tmp_work3[i];
            p_funcs_args_payload[i].arg4d = cpy_gamma;
            cpy_l = p_tmp_work4[i];
            p_funcs_args_payload[i].arg5d = cpy_l;
            start = gms::common::rdtsc_serialized_start();
            result = dqage(p_integrand,-3.141592653589793238462643383,+3.141592653589793238462643383,p_epsabs[0],p_epsrel[0],p_irule[0],&p_abser[i],
                                        &p_neval[i],&p_ier[i],&p_last[i],&p_funcs_args_payload[i]);
            end = gms::common::rdtsc_serialized_stop();
            p_functional[i] = 0.159154943091895335768883763*result;
            p_crude_tsc_start[i] = start;
...
std::int32_t 
gms::fading_channel
::compute_functional_Rice_LaplaceT_chan_5_41(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
...
#pragma omp threadprivate(seed_func_l)
    ...
    thread_local   std::uint64_t seed_func_l{};
       ...
       seed_func_psi = __rdtsc();
       rv_func_psi_gen = std::mt19937(seed_func_psi);
       rv_func_n = std::uniform_real_distribution<double>(rand_low5,rand_high5);
       seed_func_n = __rdtsc();
       rv_func_n_gen = std::mt19937(seed_func_n);
       rv_func_gamma = std::uniform_real_distribution<double>(rand_low3,rand_high3);
       seed_func_gamma = __rdtsc();
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
        p_omp_loop_start[0] = gms::common::rdtsc_serialized_start();
#pragma omp parallel for default(none) private(i,cpy_b,cpy_psi,cpy_n,cpy_gamma,cpy_l,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work5,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg2d = cpy_psi;
            cpy_n = p_tmp_work5[i];
            p_funcs_args_payload[i].arg3d = cpy_n;
            cpy_gamma = p_tmp_work3[i];
            p_funcs_args_payload[i].arg4d = cpy_gamma;
            cpy_l = p_tmp_work4[i];
            p_funcs_args_payload[i].arg5d = cpy_l;
...
std::int32_t 
gms::fading_channel
::compute_functional_Nakagami_m_LaplaceT_chan_5_43(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
...
#pragma omp threadprivate(seed_func_l)
    ...
    thread_local   std::uint64_t seed_func_l{};
       ...
       seed_func_psi = __rdtsc();
       rv_func_psi_gen = std::mt19937(seed_func_psi);
       rv_func_m = std::uniform_real_distribution<double>(rand_low5,rand_high5);
       seed_func_m = __rdtsc();
       rv_func_m_gen = std::mt19937(seed_func_m);
       rv_func_gamma = std::uniform_real_distribution<double>(rand_low3,rand_high3);
       seed_func_gamma = __rdtsc();
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
        p_omp_loop_start[0] = gms::common::rdtsc_serialized_start();
#pragma omp parallel for default(none) private(i,cpy_b,cpy_psi,cpy_m,cpy_gamma,cpy_l,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work5,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg2d = cpy_psi;
            cpy_m = p_tmp_work5[i];
            p_funcs_args_payload[i].arg3d = cpy_m;
            cpy_gamma = p_tmp_work3[i];
            p_funcs_args_payload[i].arg4d = cpy_gamma;
            cpy_l = p_tmp_work4[i];
            p_funcs_args_payload[i].arg5d = cpy_l;
...
std::int32_t 
gms::fading_channel
::compute_functional_LogNormShadow_LaplaceT_chan_5_44(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
        p_omp_loop_start[0] = gms::common::rdtsc_serialized_start();
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_b,cpy_psi,cpy_mu,cpy_sigma,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work5,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               rand_low5,rand_high5,p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
        for(i = 0; i<nfunc_vals; ++i)
...
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_b,cpy_psi,cpy_mu,cpy_sigma,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work5,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               rand_low5,p_inf,p_integrand,p_epsabs,p_epsrel,p_abser,\
               p_neval,p_ier,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
        for(i = 0; i<nfunc_vals; ++i)
...
#pragma omp parallel for default(none) private(i,cpy_theta,cpy_b,cpy_psi,cpy_mu,cpy_sigma,start,result,end) \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work5,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               rand_low5,rand_high5,p_integrand,p_epsabs,p_epsrel,p_abser,\
               p_neval,p_ier,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
#endif
        for(i = 0; i<nfunc_vals; ++i)
...
std::int32_t 
gms::fading_channel
::compute_functional_Hoyt_LaplaceT_chan_5_56(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
...
#pragma omp threadprivate(seed_func_l)
       ...
       seed_func_b = __rdtsc();
       rv_func_b_gen = std::mt19937(seed_func_b);
       rv_func_q = std::uniform_real_distribution<double>(rand_low2,rand_high2);
       seed_func_q = __rdtsc();
       rv_func_q_gen = std::mt19937(seed_func_q);
       rv_func_gamma = std::uniform_real_distribution<double>(rand_low3,rand_high3);
       seed_func_gamma = __rdtsc();
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
        p_omp_loop_start[0] = gms::common::rdtsc_serialized_start();
#pragma omp parallel for default(none) private(i,cpy_b,cpy_q,cpy_gamma,cpy_l,start,result,end,  \
                                               left_factor,l_pow,right_factor,full_factor)      \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            p_funcs_args_payload[i].arg1d = cpy_b;
            cpy_q = p_tmp_work2[i];
            p_funcs_args_payload[i].arg2d = cpy_q;
            cpy_gamma = p_tmp_work3[i];
            p_funcs_args_payload[i].arg3d = cpy_gamma;
            cpy_l = p_tmp_work4[i];
            p_funcs_args_payload[i].arg4d = cpy_l;
            ...
            left_factor  =  0.5*(cpy_b*cpy_b*cpy_gamma);
            right_factor =  (1.0+cpy_q*cpy_q)/cpy_q;
            l_pow        =  std::pow(left_factor,cpy_l);
            full_factor  =  cpy_l*left_factor*right_factor;
...
std::int32_t 
gms::fading_channel
::compute_functional_Rice_LaplaceT_chan_5_57(quadpack_integrator_payload_ch5_t * __restrict__ p_payload)
{
    if(__builtin_expect(nullptr==p_payload,0)) { return (-1);}
    double (*p_integrand)(const double,void * __restrict__)  = p_payload->integrand;
    func_args_ch5_payload_t  * __restrict__ p_funcs_args_payload = p_payload->func_args_payload;
...
#pragma omp threadprivate(seed_func_l)
       ...
       seed_func_b = __rdtsc();
       rv_func_b_gen = std::mt19937(seed_func_b);
       rv_func_n = std::uniform_real_distribution<double>(rand_low2,rand_high2);
       seed_func_n = __rdtsc();
       rv_func_n_gen = std::mt19937(seed_func_n);
       rv_func_gamma = std::uniform_real_distribution<double>(rand_low3,rand_high3);
       seed_func_gamma = __rdtsc();
...
#if (COMPUTE_FUNCTIONALS_CH5_PARALLELIZE_QUADPACK_CALLS) == 1
        p_omp_loop_start[0] = gms::common::rdtsc_serialized_start();
#pragma omp parallel for default(none) private(i,cpy_b,cpy_n,cpy_gamma,cpy_l,start,result,end,  \
                                               left_factor,mid_factor,exp_val,pow_factor,full_factor)      \
        shared(nfunc_vals,p_tmp_work1,p_tmp_work2,p_tmp_work3,p_tmp_work4,p_funcs_args_payload, \
               p_integrand,p_epsabs,p_epsrel,p_irule,p_abser,\
               p_neval,p_ier,p_last,p_functional,p_crude_tsc_start,p_crude_tsc_end,p_crude_tsc_meter) \
        schedule(static) num_threads(n_threads) proc_bind(spread) if(nfunc_vals>=n_threads)
        ...
        for(i = 0; i<nfunc_vals; ++i)
        {
            ...
            left_factor  =  0.5*(cpy_b*cpy_b*cpy_gamma);
            mid_factor   =  1.0+cpy_n*cpy_n;
            exp_val      = std::exp(-cpy_n*cpy_n);
            pow_factor   = (cpy_l+cpy_l)*std::pow(left_factor,cpy_l);
            full_factor  = pow_factor*mid_factor*exp_val;
            p_functional[i] = full_factor*result;
            p_crude_tsc_start[i] = start;
            p_crude_tsc_end[i]   = end;
            p_crude_tsc_meter[i] = end-start;
