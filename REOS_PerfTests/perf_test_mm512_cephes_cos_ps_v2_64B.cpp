...
/*
   icpc -o perf_test_mm512_cephes_cos_ps_v2_64B -O3 -fp-model fast=2 -fno-exceptions -fopenmp  -std=c++17 -ftz -ggdb -ipo -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_ansi_colors.h GMS_simd_cephes_cos.h perf_test_mm512_cephes_cos_ps_v2_64B.cpp
   ASM: 
   icpc -S --O3 fverbose-asm -masm=intel  -fno-exceptions -fopenmp  -std=c++17 -march=skylake-avx512 -mavx512f -falign-functions=32 GMS_config.h GMS_ansi_colors.h  GMS_simd_cephes_cos.h perf_test_mm512_cephes_cos_ps_v2_64B.cpp
...
void perf_test_mm512_cephes_cos_ps_v2_64B(__m512,
                                       __m512 * __restrict__,
...
__attribute__((hot))
__attribute__((noinline))
void perf_test_mm512_cephes_cos_ps_v2_64B(__m512 cos_args,
...
*/
    ...
    warmup_cos_values = gms::math::_mm512_ceph_cosf_ps_v2<true,true>(a_cos_args);
    ...
    for(int32_t __i{0}; __i != n_runs; ++__i)
    {
        ...
        {   
            ...
            result_cos_values = gms::math::_mm512_ceph_cosf_ps_v2<false,true>(a_cos_args);
...
__attribute__((hot))
__attribute__((noinline))
void perf_test_svml_mm512_cos_ps_64B(__m512,
                                    __m512 * __restrict__,
                                    unsigned __int64 * __restrict__ ,
                                    unsigned __int64 * __restrict__ ,
                                    unsigned __int64 * __restrict__ ,
...
__attribute__((hot))
__attribute__((noinline))
void perf_test_svml_mm512_cos_ps_64B(__m512 cos_args,
                                    __m512 * __restrict__ cos_values,
...
*/
    ...
    __m512 a_cos_args = cos_args;
    warmup_cos_values = _mm512_cos_ps(a_cos_args);
    ...
    for(int32_t __i{0}; __i != n_runs; ++__i)
    {
        ...
        {   
            __asm__ __volatile__ ("lfence");
            unsigned __int64 start_curr{__rdtscp(&mem_start)};
            result_cos_values = _mm512_cos_ps(a_cos_args);
            unsigned __int64 end_curr{__rdtscp(&mem_end)};
            __asm__ __volatile__ ("lfence");
            //remove latency
            unsigned __int64 start_corrected{start_curr-RDTSCP_LAT};
...
#pragma omp parallel sections 
{
    #pragma omp section 
    {
        perf_test_svml_mm512_cos_ps_64B(in_vec_cos_arg,
                                        &svml_cos_results,
                                        &svml_cos_ps_s[0],
                                        &svml_cos_ps_e[0],
                                        &svml_cos_ps_d[0],
    ...
    #pragma omp section 
    {
        perf_test_mm512_cephes_cos_ps_v2_64B(in_vec_cos_arg,
                                          &cephes_cos_results,
                                          &cephes_cos_ps_s[0],
                                          &cephes_cos_ps_e[0],
                                          &cephes_cos_ps_d[0],
     ...
     printf(ANSI_COLOR_WHITE "[PERF-TEST]:  _mm512_cephes_cosf_ps: Started!!\n");
     ...
     printf(ANSI_COLOR_BLUE "[PERF-TEST]: _mm512_cephes_cosf_ps_v2<true,true>: Finished -- dumping-results" ANSI_RESET_ALL"\n\n");
     printf(ANSI_COLOR_MAGENTA "[PERF-TEST]: Result Comparison (using safe floating-point compare as an integer)\n");
     const float * __restrict__ p_svml_cos_results   = reinterpret_cast<const float * __restrict__>(&svml_cos_results);
     ...
     {
        const float svml_result   = p_svml_cos_results[i];
        const float cephes_result = p_cephes_cos_results[i];
        const bool fcomp_result    = almostEqual2sComplement(svml_result,cephes_result,7);
        if(!fcomp_result) 
        {
            ...
            printNumber("_mm512_cephes_cosf_ps",cephes_result,0);
        }
        else
        {
